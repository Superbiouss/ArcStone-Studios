...
export function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (budgetType === "Custom") {
      finalBudget = `${currency} ${formData.get("customBudget")}`;
    } else {
      finalBudget = `${currency} ${budgetType}`;
    }
          ...
          <div>
            ...
            >
              ...
              <ScrollReveal delay={0.4}>
                <div className="flex flex-col md:flex-row gap-4 md:gap-8 border-b-2 border-border focus-within:border-accent focus-within:bg-accent/5 transition-colors">
                  <div className="w-full md:w-1/3">
                    <label htmlFor="contact-currency" className="sr-only">Currency</label>
                    <select
                      name="currency"
                      value={currency}
                      ...
                      id="contact-currency"
                    >
                      <option value="USD $" className="bg-background text-foreground text-lg">USD $</option>
                      <option value="INR ₹" className="bg-background text-foreground text-lg">INR ₹</option>
                      <option value="EUR €" className="bg-background text-foreground text-lg">EUR €</option>
                      <option value="GBP £" className="bg-background text-foreground text-lg">GBP £</option>
                    </select>
                  </div>
                  ...
                  <div className="w-full md:w-2/3">
                    ...
                    >
                      ...
                      <option value="Custom" className="bg-background text-foreground text-lg">Custom Amount</option>
                    </select>
                  </div>
                </div>
                
                {/* Conditionally render custom budget input */}
                {budgetType === "Custom" && (
                  <div className="border-b-2 border-border focus-within:border-accent focus-within:bg-accent/5 transition-colors mt-0">
                    <label htmlFor="contact-custom-budget" className="sr-only">Custom Budget</label>
                    <input
                      type="text"
                      name="customBudget"
